...
describe('Тестируем редьюсер конструктора', () => {
  it('начальное состояние', () => {
    expect(constructorReducer(undefined, {} as TConstructorActions)).toEqual(constructorInitialState);
  });
  it('определение цены заказа', () => {
    expect(constructorReducer(constructorInitialState, { type: SET_TOTAL_PRICE, totalPrice: 42 })).toEqual({
      ...constructorInitialState,
      totalPrice: 42,
    });
  });
  it('сброс цены заказа', () => {
    expect(constructorReducer(constructorInitialState, { type: RESET_TOTAL_PRICE })).toEqual({
      ...constructorInitialState,
    });
  });
  it('добавление ингредиента в заказ', () => {
    expect(constructorReducer(constructorInitialState, { type: ADD_INGREDIENT, item: ingredients[0] })).toEqual({
      ...constructorInitialState,
  ...
  it('удаление ингредиента', () => {
    ...
    expect(
      constructorReducer(
        { ...constructorInitialState, ingredientsConstructor },
        { type: DELETE_INGREDIENT, uid: deletedIngredient.uid },
      ),
  ...
  it('перетаскивание ингридиента в конструкторе', () => {
    ...
    expect(
      constructorReducer(
        { ...constructorInitialState, ingredientsConstructor },
        { type: MOVE_INGREDIENT, sourceIndex: 1, targetIndex: 0 },
      ),
    ).toEqual({ ...constructorInitialState, ingredientsConstructor: result });
  });
  it('сброс ингредиентов в конструкторе', () => {
    const ingredientsConstructor = ingredients;
    expect(
      constructorReducer({ ...constructorInitialState, ingredientsConstructor }, { type: RESET_INGREDIENTS }),
