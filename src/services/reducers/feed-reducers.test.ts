...
describe("Тестирование редьюсера ленты заказов", () => {
  it("возвращение начального состояния", () => {
    expect(feedOrdersReducer(undefined, {} as TFeedOrdersActions))
  ...
  it("подключение к вебсокетам с общей лентой заказов успешно", () => {
    ...
    .toEqual({...feedOrdersInitialState, wsConnected: true})
  });
  ...
  it("закрытие подключения к вебсокетам с общей лентой заказов", () => {
    expect(feedOrdersReducer(
      {...feedOrdersInitialState, wsConnected: true},
  ...
  it("получение ленты заказов", () => {
    expect(feedOrdersReducer(
      {...feedOrdersInitialState, wsConnected: true},
      {type: WS_GET_ORDERS, orders}))
    .toEqual({...feedOrdersInitialState, wsConnected: true, orders: orders.orders, total: orders.total, totalToday: orders.totalToday})
  });
  it("получение ленты заказов пользователя", () => {
    expect(feedOrdersReducer(
      {...feedOrdersInitialState, wsConnected: true},
      {type: WS_GET_ORDERS_USER, orders}))
